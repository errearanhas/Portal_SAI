...
def get_docx_files(folder):
    """
    looks at the folder with documents and searches for docx files
    """
    doc_list = []
    docx_list = []
...
def generate_corpus(txt_output_file, docx_list):
    """
    generates a corpus from the docx files and exports as a txt file
    """
    corpus = []
...
def load_corpus_file(txt_file):
    """
    loads a txt corpus file, if it already exists
    """
    # LOAD CORPUS FILE
...
def replaces_in_corpus(corpus, topic='RPT'):
    """
    applies manual term replacements in corpus
    """
...
blacklist_conflict = ['_','–',' ','Augusto','augusto','Carlos' "$",")","(",'/2015',"walton",'alencar','alves',".","(...)",
...
blacklist_parties = ['_','–',' ','Augusto','augusto','Carlos' "$",")","(",'/2015',"walton",'alencar','alves',".","(...)","[...]",
...
blacklist_nepotism = ['_','–',' ','Augusto','augusto','Carlos' "$",")","(",'/2015',"walton",'alencar','alves',".","(...)","[...]", 'sobre',
...
def remove_blacklist(corpus, blacklist):
    """
    removes a previously defined blacklist from the corpus, and also removes punctuation, stopwords and shortwords
    """
...
def filter_text_chunks(text_total, topic='RPT'):
    """
    gets text within the selected topic: RELATED PARTIES, NEPOTISM or CONFLICTS OF INTEREST.
    optionally, can also apply user defined list of terms about 'RELATED PARTIES' (based on domain knowledge), in order
    to explicit consider chunks of text that include them.
    """
...
def generate_lda(total_topics, text_term, text_total, which_text='TOTAL'):
    """
    LDA corpus generation based on choice of entire corpus data or filtered corpus data
    """
    if which_text == 'TERM':
        dict_term = corpora.Dictionary(text_term)
        corpus_term = [dict_term.doc2bow(i) for i in text_term]
...
def show_N_terms(lda_term, lda_total, total_topics):
    """
    shows terms by topic, in relevance order
    """
...
def prepare_data_clustermap(lda_term, lda_total, total_topics):
    """
    generates input dataframes for posterior cluster map generation
    """
...
def generate_clustermap(df_lda_total, df_lda_term, which_text='TOTAL'):
    """
    generates cluster map, based on entire data or on filtered corpus data
    """
    if which_text=='TERM':
...
def generate_lda_vis(lda_term, corpus_lda_term, dict_term):
    """
    Generates intertopic distance map
    """
